...
def menus_update(request, restaurant_pk, menu_pk):
    ...
    if request.method == 'POST':
        menu_form = MenuForm(request.POST, instance=menu, hide_name=True)
        if menu_form.is_valid():
            menu = menu_form.save(commit=False)
            menu.restaurant = restaurant
            menu.save()
        ...
        menu_form = MenuForm(instance=menu, hide_name=True)
    context = {
        'restaurant': restaurant,
        'menu' : menu,
        'menu_form': menu_form
    }
    return render(request, 'restaurants/menus_update.html', context)